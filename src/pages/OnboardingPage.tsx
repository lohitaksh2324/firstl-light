...
export default function OnboardingPage() {
  ...
  const finishOnboarding = () => {
    ...
    localStorage.setItem('fl_auth', JSON.stringify({ ...auth, onboarded: true }));
    navigate('/dashboard');
  };
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-[#0A0C0F] text-white flex flex-col">
        {/* Progress bar */}
        <div className="w-full h-1 bg-[#1E2530]">
          <motion.div className="h-full bg-[#F97316]" initial={{ width: 0 }} animate={{ width: `${(step / 4) * 100}%` }} />
        </div>
        <div className="text-center text-xs text-[#64748B] pt-2">Step {step} of 4</div>
        ...
        <div className="flex-1 flex flex-col relative pb-24">
          ...
          {step === 1 && (
            <div className="max-w-4xl mx-auto pt-16 px-8 w-full">
              <h1 className="font-display text-4xl font-bold text-center">Choose Your Track</h1>
              <p className="text-[#64748B] text-center mt-2 mb-10">Select the domain you want to master — your simulations will be tailored to it</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {tracks.map(t => {
                  const isSelected = selectedTrack === t.id;
                  return (
                    <motion.div key={t.id} whileHover={{ y: -4 }} onClick={() => setSelectedTrack(t.id)}
                      className={`relative bg-[#111318] rounded-xl p-7 flex flex-col items-center text-center cursor-pointer transition-all border ${isSelected ? 'glow-orange' : 'border-[#1E2530] hover:border-[#2E3540]'}`}
                      ...
                      <div className="font-display font-semibold mt-3 text-sm">{t.label}</div>
                      <div className="text-xs text-[#64748B] mt-1 leading-relaxed">{t.desc}</div>
                    </motion.div>
                  );
                })}
              </div>
          ...
          {step === 2 && (
            <div className="max-w-xl mx-auto pt-16 px-8 w-full">
              ...
              <div className="h-20 flex items-center justify-center mb-6">
                <AnimatePresence mode="wait">
                  <motion.h2 key={currentQ} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} transition={{ duration: 0.2 }}
                    className="font-display text-xl font-semibold text-center text-white">
                ...
                <AnimatePresence mode="wait">
                  {trackQuestions[currentQ]?.options.map((opt, i) => (
                    <motion.button key={`${currentQ}-${i}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                      ...
                      className="w-full p-4 rounded-xl border border-[#1E2530] bg-[#111318] text-left hover:border-[#F97316] hover:bg-[rgba(249,115,22,0.06)] transition-all group flex items-center gap-3">
              ...
              <div className="flex justify-center gap-2 mt-8">
                {[0, 1, 2, 3, 4].map(q => (
                  <div key={q} className={`transition-all rounded-full ${q === currentQ ? 'w-6 h-2' : 'w-2 h-2'} ${q < currentQ ? 'bg-[#4ADE80]' : q === currentQ ? 'bg-[#F97316]' : 'bg-[#1E2530]'}`} />
                ))}
              </div>
              <div className="text-center text-xs text-[#64748B] mt-2">Question {currentQ + 1} of 5</div>
          ...
          {step === 3 && (
            <div className="max-w-4xl mx-auto pt-16 px-8 w-full">
              <h1 className="font-display text-4xl font-bold text-center">What is Your Primary Goal?</h1>
              <p className="text-[#64748B] text-center mt-2 mb-12">Your path will be optimised for your objective</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {goals.map(g => {
                  const isSelected = selectedGoal === g.id;
                  return (
                    <motion.div key={g.id} whileHover={{ y: -4, borderColor: "#F97316" }}
                      onClick={() => setSelectedGoal(g.id)}
                      className={`relative bg-[#111318] rounded-xl p-8 flex flex-col items-center text-center cursor-pointer transition-all border ${isSelected ? 'border-[#F97316] bg-[rgba(249,115,22,0.05)] glow-orange' : 'border-[#1E2530]'}`}>
                      {isSelected && <CheckCircle className="absolute top-4 right-4 w-4 h-4 text-[#4ADE80]" />}
                      <g.icon className={`w-12 h-12 ${isSelected ? 'text-[#F97316]' : 'text-[#64748B]'}`} />
                      <div className="font-display font-semibold mt-4 text-lg">{g.label}</div>
                      <div className="text-sm text-[#64748B] mt-2 leading-relaxed">{g.desc}</div>
                    </motion.div>
                  );
                })}
              </div>
          ...
          {step === 4 && (
            <div className="max-w-2xl mx-auto pt-16 px-8 w-full">
              ...
              <h1 className="font-display text-4xl font-bold text-center text-white">Your Path is Ready</h1>
              <p className="text-[#64748B] text-center mt-2">Personalised to your track, skill level, and goal</p>
              ...
              <div className="relative max-w-lg mx-auto mt-12">
                ...
                <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.15 } } }} className="space-y-0">
                  {trackPath.map((node, i) => (
                    <motion.div key={i}
                      variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0 } }}
                      className="flex items-start gap-4 relative pl-12 pb-8">
                      <div className="absolute left-0 flex items-center justify-center w-8 h-8 rounded-full z-10 bg-[#0A0C0F]">
                        ...
                        {node.state === 'current' && (
                          <div className="w-full h-full rounded-full flex items-center justify-center pulse-ring bg-[#0A0C0F]"
                        ...
                        {node.state === 'locked' && <div className="w-full h-full rounded-full border-2 border-[#1E2530] flex items-center justify-center bg-[#0A0C0F]"><Lock className="w-3.5 h-3.5 text-[#64748B]" /></div>}
                      ...
                      <div className={`pt-0.5 ${node.state === 'final' ? 'pt-1' : ''}`}>
                        <div className={`font-display font-semibold text-sm ${node.state === 'locked' ? 'text-[#64748B]' : node.state === 'final' ? 'text-[#F59E0B]' : 'text-white'}`}>{node.title}</div>
                        {node.time && (
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-xs text-[#64748B]">{node.time}</span>
                          </div>
                        )}
                        {node.state === 'current' && (
              ...
              {/* Skill snapshot */}
              {answers.length > 0 && (
                <div className="mt-4 bg-[#111318] border border-[#1E2530] rounded-xl p-4">
                  <div className="text-xs text-[#64748B] uppercase tracking-widest mb-3">Your Skill Snapshot</div>
                  <div className="space-y-2">
                    {trackQuestions.slice(0, 5).map((q, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className="text-[10px] text-[#94A3B8] flex-1 truncate">{q.question.split(' ').slice(0,5).join(' ')}…</div>
        ...
        {/* Bottom Nav */}
        <div className="fixed bottom-0 left-0 right-0 bg-[rgba(10,12,15,0.95)] backdrop-blur border-t border-[#1E2530] px-8 py-4 flex justify-between items-center">
          {step > 1 && step < 4 && (
            <button onClick={() => { setStep(s => s - 1); if (step === 3) { setCurrentQ(0); setAnswers([]); } }}
              className="border border-[#1E2530] hover:bg-[#181C23] text-[#64748B] hover:text-white font-display px-6 py-2.5 rounded-xl text-sm transition-colors">
              ← Back
            </button>
          )}
          {step === 1 && <div />}
          ...
          {step < 3 && (
            <button onClick={handleNext}
              ...
              className={`ml-auto font-display font-semibold px-8 py-2.5 rounded-xl text-sm transition-all ${(step === 1 && !selectedTrack) ? 'bg-[#1E2530] text-[#64748B] cursor-not-allowed' : 'bg-[#F97316] hover:bg-[#F97316]/90 text-white'}`}>
              {step === 1 ? 'Continue →' : 'Next →'}
            </button>
          )}
          {step === 3 && (
            <button onClick={handleNext} disabled={!selectedGoal}
              className={`ml-auto font-display font-semibold px-8 py-2.5 rounded-xl text-sm transition-all ${!selectedGoal ? 'bg-[#1E2530] text-[#64748B] cursor-not-allowed' : 'bg-[#F97316] hover:bg-[#F97316]/90 text-white'}`}>
              Build My Path →
            </button>
          )}
          {step === 4 && (
            <button onClick={finishOnboarding} className="ml-auto bg-[#F97316] hover:bg-[#F97316]/90 text-white font-display font-semibold px-10 py-3 rounded-xl text-sm transition-colors">
              Enter the Forge →
            </button>
          )}
        </div>
